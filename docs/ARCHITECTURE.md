...
- **rules/analyzers** apply the heuristics shipped in `tokenrotate/core.py`.
- **score** ranks by severity.
- **MCP server** (`tokenrotate mcp`) exposes `scan` for Cognis.Studio agents.
